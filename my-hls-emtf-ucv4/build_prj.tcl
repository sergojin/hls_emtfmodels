...
if {$opt(reset)} {
  open_project -reset emtfptnn_prj
} else {
  open_project emtfptnn_prj
}
set_top emtfptnn
add_files firmware/emtfptnn.cpp -cflags "-std=c++0x"
add_files -tb emtfptnn_test.cpp -cflags "-std=c++0x"
...
if {$opt(cosim)} {
  ...
  add_files -tb emtfptnn_test.cpp -cflags "-std=c++0x -DRTL_SIM"
  set time_start [clock clicks -milliseconds]
  cosim_design -trace_level all
  set time_end [clock clicks -milliseconds]
  puts "INFO:"
  puts [read [open emtfptnn_prj/solution1/sim/report/emtfptnn_cosim.rpt r]]
...
if {$opt(vsynth)} {
  ...
  if {[file exist emtfptnn_prj/solution1/syn/vhdl]} {
    set time_start [clock clicks -milliseconds]
